...
\subsection{Stop}
Finding \textit{Stops} is done by traversing every \textit{Single Location Points} in temporal order, i.e. the timestamp is used. The \textit{Stops} for a given date is found by clustering \text{Single Location Points} on that date based on time and distance. The set of \textit{Stops} found for the period $D$ is defined as
...
is a set of time-ordered \textit{SLPs}. Moves are found using the following procedure:
